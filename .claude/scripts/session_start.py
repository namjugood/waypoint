...
ORPHAN_IDLE_SECONDS = 3 * 60 * 60  # 이보다 오래 안 건드려진 원문로그만 "죽은 세션"으로 간주

# 10분(과거 값)은 너무 짧았다 — 실제로 겪은 사고: 사용자가 답장을 잠깐
# 늦게 했을 뿐인, 아직 살아있는 세션(160329fb, 2026-09-09)이 *다른*
# 세션의 orphan 복구에 의해 두 번이나(06:08, 06:19) "완료 처리"됐다.
# finalize_session이 구조화 임시본(structured.md)을 tags/로 옮기고
# 지워버리면, 그 뒤에도 원래 세션이 계속 이어지면서 다시 체크포인트가
# 돌 때 연속성 메타(primary_tag/file_ts)를 잃어버려 매번 새 file_ts를
# 만들고 -> INDEX.md에 고아 링크가 쌓이는 문제로 이어졌다(원래
# state.json 유실 버그를 고친 뒤에도 이 경로로 똑같은 증상이 재발함).
# 살아있는 세션을 죽었다고 오판하는 비용(며칠 뒤에나 없어지는 게 아니라
# 몇 시간 늦게 정리되는 정도)이, 살아있는 세션을 잘못 끊어버리는 비용보다
# 훨씬 싸다 — 그래서 넉넉하게 3시간으로 늘렸다. 근본적으로는 "무활동
# 시간"만으로 죽음을 판단하는 방식 자체의 한계이므로(하트비트 등 별도
# 신호가 없는 한), 값을 늘리는 건 완화책이지 완전한 해결책은 아니다.
